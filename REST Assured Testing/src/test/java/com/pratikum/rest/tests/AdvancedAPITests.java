...
import com.pratikum.rest.utils.TestDataGenerator;
 ...
 * Mengcover data-driven testing, negative testing, performance testing, all.
 */
public class AdvancedAPITests extends BaseTest {
    ...
    public void testUpdateUserWithPartialData() {
        ...
        partialData.put("name", "Updated Name Only"); // Update hanya name

        given()
                .contentType(ContentType.JSON)
                .pathParam("id", 1)
                .body(partialData) // Only send fields to update
                .when()
                .patch("/users/{id}") // PATCH request untuk partial update
                .then()
                .statusCode(200)
                .body("name", equalTo("Updated Name Only")) // Validate name terupdate
                .body("id", equalTo(1)); // Validate ID unchanged
    }
    ...
    public void testCreateUserWithNullValues() {
        ...
        given()
                ...
                .post("/users")
                .then()
                .statusCode(201) // JSONPlaceholder handles null values
