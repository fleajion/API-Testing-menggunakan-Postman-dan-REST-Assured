 ...
 */
public class AuthenticationTests { // Removed extends BaseTest karena kelas BaseTest tidak ada
    ...
    public void setupMethod() {
        // Menggunakan JSONPlaceholder API sebagai base
        baseURI = "https://jsonplaceholder.typicode.com";
        useJSONPlaceholderAPI();
    }

    /**
     * Method untuk menggunakan JSONPlaceholder API
     */
    private void useJSONPlaceholderAPI() {
        // Set base URI untuk RestAssured
        io.restassured.RestAssured.baseURI = "https://jsonplaceholder.typicode.com";
    }

    /**
     * Method untuk menggunakan RegRes API (dummy implementation)
     */
    private void useRegResAPI() {
        // Set base URI untuk RestAssured ke RegRes API
        // Ganti dengan URL API RegRes yang sebenarnya
    ...
    public void testCreatePostWithoutAuth() {
        // Menggunakan text block untuk string JSON yang lebih bersih
        String postBody = """
            {
                "title": "Test Post",
    ...
    public void testWithMalformedData() {
        given()
                ...
                .body("id", notNullValue());   // generate ID
    }
    ...
    public void testSuccessfulLoginMock() {
        ...
        // kita menggunakan GET user sebagai proxy untuk request
        given()
                .contentType(ContentType.JSON)
                .when()
    ...
    public void testFailedAuthenticationMock() {
        given()
                ...
                .then()
                .statusCode(404);    // Validate Not Found
    }

    /**
     * Optional test untuk check RegRes API availability
     * Dengan error handling dengan fungsi skip jika API down
     */
    @Test
    public void testRegResAPIAvailability() {
        try {
            // Switch sementara ke RegRes API
            useRegResAPI();
            given()
                    .contentType(ContentType.JSON)
                    .when()
                    .get("/users") // GET request ke RegRes API
                    .then()
